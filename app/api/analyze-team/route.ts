...
const SYSTEM_PROMPT = `あなたはポケモンチャンピオンズ (ポケチャン) の「チーム能力確認画面」のスクリーンショットを解析するアシスタントです。
...
export async function POST(req: Request) {
  ...
  const normalized = normalizeAnalyzedTeam(parsed);
...
function normalizeAnalyzedTeam(input: unknown): unknown {
  ...
  const normalizedPokemons = team.pokemons.map((p) => {
    const out: Record<string, unknown> = { ...p };

    // ポケモン名: まず resolvePokemonJaName (厳密)、ダメなら findBestJaMatch (ファジー距離2)
    ...
    // 特性
    if (typeof p.ability === "string" && p.ability.trim()) {
      const fuzzy = findBestJaMatch(p.ability, ABILITY_JA_LIST, { maxDistance: 2 });
      if (fuzzy) out.ability = fuzzy;
    }

    // 持ち物
    if (typeof p.item === "string" && p.item.trim()) {
      const fuzzy = findBestJaMatch(p.item, ITEM_JA_LIST, { maxDistance: 2 });
      if (fuzzy) out.item = fuzzy;
    }

    // 技リスト
    if (Array.isArray(p.moves)) {
      out.moves = p.moves.map((m) => {
        if (typeof m !== "string" || !m.trim()) return m;
        const fuzzy = findBestJaMatch(m, MOVE_JA_LIST, { maxDistance: 2 });
        return fuzzy ?? m;
      });
    }
