...
各パネルは縦に左右2分割されており、配置は厳密に以下の通りです:

  ┌─────────────────────────┬─────────────────────────┐
  │ 【左半分】              │ 【右半分】              │
  │ ・ポケモン名(上部)      │ ・技1 (上から1番目)     │
  │ ・性別/テラスタイプアイコン │ ・技2 (上から2番目)     │
  │ ・特性 (中段)            │ ・技3 (上から3番目)     │
  │ ・持ち物 (下段、アイコン付き) │ ・技4 (上から4番目)     │
  └─────────────────────────┴─────────────────────────┘
...

{
  ...
  "rating": "画面に「レート」「1975.001」のような対戦レート数値 (1000〜3000の範囲、小数可) が写っていればその数値。シングルバトルとダブルバトルの両方が見える画面では、シングルの方のレートを優先。数値を読めなかった場合や見えない場合は null。文字列ではなく数値 (number) で返す",
  "pokemons": [
    {
      "slot": 1,
...
⚠ よくある誤認識に注意:
- パネルの右下隅や端に表示される 1〜6 の大きな数字はスロット番号です。技名の一部ではありません。
  特に「3」を「３ぼんのや」(=ヒスイジュナイパー専用技)の一部だと誤認しないでください。
- 技の「まもる」は全ひらがな3文字で頻出の技。4つ目の技が読めない時はまず「まもる」を疑ってください
  （プロテクトアイコン：緑色の盾マーク）。
  ★競技用構築では実に半数以上のポケモンが「まもる」を採用しています。
  読めない最後の技スロット = 第一候補は「まもる」、第二候補は「みがわり」と覚えておいてください。
...
- 「マジックミラー」(特性) と 「マジカルシャイン」(技) は別物。
- 「フェアリースキン」(特性) と 「フェアリーオーラ」(特性) は別物。
...
// ヒスイジュナイパー専用の３ぼんのや等、競技でほぼ使われない技は誤認の温床なので除外。
// もし本当に必要なケースが出たら admin で手動入力すれば良い。
