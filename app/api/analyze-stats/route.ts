...
画面には 6 体のポケモンが 2列×3行 のグリッドで並んでいます。
スロット番号の割り当て:
  左上=スロット1, 右上=スロット2
  左中=スロット3, 右中=スロット4
  左下=スロット5, 右下=スロット6

各パネルには以下のステータス情報が表示されます:
- ポケモン名 (日本語)
- ステータスは各パネル内で 2列×3行 に配置されています。位置を厳密に守ってください:
  ┌──────────────────────────────────────┐
  │ ♥ HP    実数値 努力値 │ ✿ とくこう 実数値 努力値 │  ← 1行目
  │ ✱ こうげき 実数値 努力値 │ ◎ とくぼう 実数値 努力値 │  ← 2行目
  │ ✱ ぼうぎょ 実数値 努力値 │ ≋ すばやさ 実数値 努力値 │  ← 3行目
  └──────────────────────────────────────┘
  左列1行目=HP, 左列2行目=こうげき(attack), 左列3行目=ぼうぎょ(defense)
  右列1行目=とくこう(spAtk), 右列2行目=とくぼう(spDef), 右列3行目=すばやさ(speed)

  ⚠ とくぼう(右列2行目)とすばやさ(右列3行目)を絶対に取り違えないでください。
  すばやさは必ず右列の一番下(3行目)です。

- 各ステータスの数値の読み方:
  「ラベル名 実数値 ── 努力値」の形式です
  実数値 = 大きい3桁の数字(例: 187)
  努力値 = バー横の小さい数字(0〜32の範囲、例: 20)
  実数値と努力値を取り違えないでください
...
{
  "rating": "画面に「レート」や「1975.001」のような対戦レート数値 (1000〜3000の範囲、小数可) が写っていればその数値。シングル/ダブル両方あればシングルを優先。なければ null。文字列ではなく数値 (number) で返す",
  "pokemons": [
    {
      "slot": 1,
      "name": "ポケモンの日本語名",
