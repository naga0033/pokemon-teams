...
画面には 6 体のポケモンが 2列×3行 のグリッドで並んでおり、各パネルには以下のステータス情報が表示されます:
- ポケモン名 (日本語)
- ステータスは各パネル内で 2列×3行 に配置されています:
  左列 (上から): ♥ HP, ✱ こうげき, ✱ ぼうぎょ
  右列 (上から): ✿ とくこう, ◎ とくぼう, ✿ すばやさ
- 各ステータスは「実数値」(3桁の大きい数字) と、その右にバー付きの「努力値」(0〜32の小さい数字) がペアで表示されます
- 実数値と努力値を取り違えないでください。左側の大きい数字が実数値、右側の小さい数字が努力値です
